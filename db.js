...
if (process.env.MYSQL_URL || process.env.DATABASE_URL) {
  ...
  try {
    ...
    console.error('Failed to create connection from URL:', error.message);
    process.exit(1);
  }
} else {
...
db.connect({timeout: 30000}, err => {
  if (err) {
    ...
    // Don't exit the process - let the app continue to run so other routes still work
